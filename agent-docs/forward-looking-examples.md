...
config = TrainingConfig(
    ...
    training_steps=max(10, len(train_data) // 32),
    lora_rank=32,
    batch_size=32,
    num_rollouts=8,
...
Predict professional golf outcomes. Binary questions, news seeds, 14-day interval matching tournament cadence.
...
# Production: max_questions=10000
```
...
Predict political actions within a 2-month horizon. Weekly intervals, high questions_per_seed (20) for a narrow domain.
...
Predict military strike events at scale. Detailed instructions requiring specific actor/target naming — vague military questions are useless.
...
config = TrainingConfig(
    base_model="openai/gpt-oss-120b",
    training_steps=max(10, len(train_dataset.flattened()) // 32),
    lora_rank=32, batch_size=32, num_rollouts=8,
...
General-purpose forecasting from global news via GDELT. High confidence threshold (0.9) for production quality. Custom answer format.
...
**Training**: `gpt-oss-120b`, `num_rollouts=4` (sufficient for large general dataset), `training_steps=160`, `length_penalty_enabled=True` (keeps responses concise).

---

## Example 5: Beige Book (FileSet RAG)

Forecast district-level economic outcomes from Fed Beige Book reports. Each edition resolves questions from the previous one via `FileSetRAGLabeler` with `TemporalConstraint.AFTER`.
...
# Upload Beige Book PDFs to FileSet with date metadata
...
| `lora_rank` | 32 | Standard |
| `batch_size` | 32 | Standard |
| `num_rollouts` | 8 (domain), 4 (general) | More = better signal |
| `max_response_length` | 16384 | Reasoning chains |
| `learning_rate` | 4e-5 | Standard |
| `training_steps` | `ceil(train_rows / 32)` | One pass |
